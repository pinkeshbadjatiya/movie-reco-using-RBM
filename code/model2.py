...
class baseline_tensor():
    ...
    def __init__(self):
        ...
        self.R_train_ui = dict(zip(zip(self.R_train[:, 0], self.R_train[:, 1]), self.R_train[:, 2])) 
        self.R_test_ui = dict(zip(zip(self.R_test[:, 0], self.R_test[:, 1]), self.R_test[:, 2]))
    ...
    def model(self, lr_a = 0.1, lr_b = 0.1, lr_c = 0.1, n_it = 150):
